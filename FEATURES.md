...
* **Frontend Path Sanitization & Merge:** Implemented frontend project sanitization and duplicate path merging on load and during thread sync. This automatically strips trailing markdown symbols (e.g., backticks, asterisks, brackets, punctuation) from legacy `localStorage` project entries and merges any duplicate paths, preventing sidebar project fragmentation.
