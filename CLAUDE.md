...
16. **Global Configuration Truth:** Any time you are asked to add, modify, or read "global rules", "customizations", or "agent configurations", you MUST perform those changes in master files (`~/projects/ai-os/AGENTS.md` for agy/Gemini and `~/projects/ai-os/CLAUDE.md` for Hermes/Claude). Note: `AGENTS.md` is a single symlink target (`/Users/matt/.gemini/GEMINI.md`) shared by `.agents/AGENTS.md`, `.gemini/GEMINI.md`, and `~/.gemini/GEMINI.md`, so updating `AGENTS.md` updates all Gemini rule locations automatically.
