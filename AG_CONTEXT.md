...
At the end of your task execution (when yielding control back to the user), you MUST execute the telemetry check script:

```bash
python3 /Users/matthewmurphy/projects/ai-os/scripts/get_last_cost.py --agent agy  # or --agent claude respectively
```
...
* **2026-06-27:** Optimized Tauri engine startup flow by launching `agy` and `claude` directly as the initial process in the PTY/tmux session, bypassing shell configuration load latency (e.g. `nvm`, `~/.zshrc`) and lowering launch times from 12s to under 2s. Removed the redundant frontend `write_to_pty` startup injection.
