...
- **Gemini Floating Webview Auto-Transformation & Pseudo-Toolbar:** Updated `floating_init_script` in `tauri-gui/src-tauri/src/main.rs` to inject a 38px macOS pseudo-toolbar with window drag regions and stoplight buttons (close, minimize, maximize) upon message submission, while switching window sizing from physical to logical units (`1280x880` CSS pixels) centered on screen for comfortable reading. [[log]](agent-logs/2026-07-22_02-35_pseudo-toolbar-stoplights-and-window-resizing.md)
