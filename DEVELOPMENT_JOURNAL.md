...
## 2026-08-19
- **Fixed Multi-Screenshot & Image Attachments across `query_aios.js` and AI-OS Companion:** Expanded `query_aios.js` to accept multiple `--screenshot` / `--image` / `-f` arguments into an array and auto-detect all image references in prompts. Upgraded `apps/gemini-companion/src-tauri/src/server.rs` payload models with `attachments` and `file_paths` array support (`prepare_attachments_list`), and updated `perplexity-engine.js` and `gemini-engine.js` to upload all attachments to S3/Google Scotty and pass complete attachment token arrays. Successfully verified end-to-end multi-image query against live Perplexity. [[log]](agent-logs/2026-08-19_17-15_query-aios-multi-screenshot-attachments.md)
...
## 2026-08-16
- **CLI Heartbeat & Perplexity Stream Fixes:** Removed periodic CLI heartbeat spam from `query_aios.js` to preserve context tokens, keeping only a single initial notification and a structured completion banner on arrival. Fixed Perplexity stream parser (`_parseStream`) termination in `perplexity-engine.js` for clean response yielding on completion (instead of blocking on HTTP keep-alive) and sanitized IPC payload parameters across `server.rs` and `main.rs`. Fixed Perplexity stream parser (`_parseStream`) in `perplexity-engine.js` so it does not terminate on intermediate search steps (where `answer` is empty and `step_type` is present), ensuring that multi-step search reasoning properly continues until the final markdown answer is generated and completed. [[log]](agent-logs/2026-08-16_22-55_ai-os-query-final-output-signaling-and-stream-fix.md)
...
- **2026-08-17 16:30 (Always-On AI-OS Companion Server Launch Agent Service)**: Fixed `server.rs` panic on address binding by setting `SO_REUSEADDR`/`SO_REUSEPORT` with a 10-attempt backoff retry loop. Created dedicated launch agent `com.matt.agent.aios-server.plist` managed via `la` (`~/.local/bin/la`) running inside named tmux session `agent-aios-server` with launchd auto-restart on crash. Added automated launch agent wake/recovery and polling to `scripts/query_aios.js`. [[log]](agent-logs/2026-08-17_16-30_aios-always-on-launch-agent-service.md)