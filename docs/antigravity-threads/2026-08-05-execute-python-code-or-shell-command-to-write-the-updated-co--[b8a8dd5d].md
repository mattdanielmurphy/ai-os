---
title: "Execute python code or shell command to write the updated contents into `/Users/matt/projects/ai-os/scripts/check_merged_prs.py`."
date: "2026-08-05"
conversation_id: "b8a8dd5d-d018-4e5f-b439-df920254ca1d"
source: "antigravity"
---

# Execute python code or shell command to write the updated contents into `/Users/matt/projects/ai-os/scripts/check_merged_prs.py`.

## User

Execute python code or shell command to write the updated contents into `/Users/matt/projects/ai-os/scripts/check_merged_prs.py`.

Use python inline command:
```bash
python3 -c '
...
def main():
    ...
    for pr_id, pr in new_prs:
        repo_name = pr.get("repository", {}).get("nameWithOwner", "")
        msg = f"🎉 Bounty PR Pulled! {repo_name}#{pr[\x27number\x27]} - {pr[\x27title\x27]} ({pr[\x27url\x27]})"
        if not args.quiet:
...
if __name__ == "__main__":
    main()
"""
with open("/Users/matt/projects/ai-os/scripts/check_merged_prs.py", "w") as f:
    f.write(code)
'
```
