---
title: "why do tv shows blur out license plates?"
date: "2026-08-13"
source: "gemini.google.com"
conversation_id: "6db1417d-1742-58f4-9457-b11906c763b1"
url: ""
---

# why do tv shows blur out license plates?

> [!NOTE]
> Archived Gemini Thread: [why do tv shows blur out license plates?]() | Date: 2026-08-13
...
It usually comes down to a mix of legal caution, strict network standards, and protecting people's privacy.


The main reasons production companies take the time to obscure them include:


1. The "Cover Your Assets" Legal Strategy
While a license plate is visible in public, it is tied to an individual's personal data (like their home address and registration details). If a TV show displays a real person's license plate—especially in a context that is negative, scandalous, or controversial (like reality TV, true crime, or news segments)—it opens the door to potential lawsuits.


Even if the network would technically win the lawsuit on public-domain grounds, paying a team of lawyers to fight it is incredibly expensive. Blurring the plate eliminates the risk entirely.


2. Preventing Doxxing and Harassment
TV shows are broadcast to millions of viewers. If a real license plate is left unblurred, it takes only one determined, tech-savvy viewer or someone with access to a database to look up the vehicle owner's identity, locate their residence, and potentially stalk or harass them.


3. Avoiding "Vehicle Cloning" and Fraud
Leaving a clear image of a legitimate license plate on a major broadcast makes it a target for criminals looking to duplicate plates. They can print a fake plate with those exact characters, put it on a matching car model, and rack up toll violations, speeding tickets, or commit crimes under someone else's registration.


4. International Privacy Regulations
For shows broadcast or streamed internationally, networks must comply with strict global data privacy standards. For example, under the European Union's GDPR (General Data Protection Regulation), a license plate is explicitly considered personally identifiable information (PII). Leaving them unblurred could result in massive fines or require the content to be restricted in certain countries.




Did you know? Scripted dramas and comedies rarely have to worry about blurring because they usually use fake prop plates cleared by the legal department (similar to the classic "555" phone numbers). Blurring is almost exclusively seen in reality TV, news broadcasts, documentaries, and unscripted street footage where tracking down every vehicle owner for a waiver is impossible.

---

## User

anti-doxing I get. But when they show a shot of literally just a normal looking vehicle or a stream of normal looking vehicles on a public road, and they're all blurred out it's like: why the fuck is this even a thing?
You can go outside ANYWHERE in ANY CITY and see thousands upon thousands of license plates so point 3 makes no sense at all.

It seems just like ridiculous over-caution that literally helps nobody ever

---

## Gemini

You have hit the nail on the head. For a massive stream of random cars on a public road, it is massive over-caution—and practically speaking, it doesn't help the average person on the street.


The disconnect here is that you are thinking like a rational person looking at public data, while network executives are thinking about clearing legal liability across 190 different countries at scale.


When a production company has a massive crew editing hours of footage, they don't have the time or budget to analyze every single car to determine if it's "safe" or "risky." Instead, they rely on blanket corporate legal policies. Here is why those ridiculous, sweeping blur-fests happen:


The "All or Nothing" Workflow
Legal departments at major networks (like Netflix, Warner Bros., or Disney) hand down strict guidelines to editing houses. It is drastically cheaper and faster to give an editor a blanket rule—"Blur every single visible plate, period"—than it is to pay an entertainment lawyer to review every frame of traffic footage to see if a specific plate is a liability risk.


If an editor misses one plate that shouldn't have been there, the network can halt production or reject the final cut. So, editors just use automated blurring tools on everything to get the project approved.


The Real Danger of Point 3: The Amplification Factor
You are exactly right that anyone can walk outside right now and see thousands of plates. The difference is scale and persistence.


If a scammer walks down your street, they get access to the 20 cars parked there. If a license plate is broadcast on a hit streaming show, it is suddenly exposed to 50 million people simultaneously, and that footage lives on the internet forever.



* A criminal looking to clone a plate for a white Ford F-150 doesn't need to walk around car lots hoping to find one; they can just pause a 4K stream of a documentary, take a screenshot of a perfect match, and go to work.

* By blurring all of them, the network protects itself from being the source of that data leak.


Global Laws Force Lowest-Common-Denominator Editing
As mentioned before, laws like the EU's GDPR changed the game. In North America, the expectation of privacy in public is very low. But in Europe, data privacy laws are absolute.


If a streaming platform wants to release a documentary globally, they have two choices:



* Create a "North American Cut" (unblurred) and an "International Cut" (blurred).

* Just blur everything from the start and distribute one single file globally.


To save money, they always choose option two. So, viewers in Canada and the US end up watching a heavily blurred stream of random traffic because a strict privacy law in Germany or France legally requires it for their viewers.


It feels like bureaucratic, automated overkill because that is exactly what it is.
...
I asked a question and it said it was searching the web.
How's it doing that?
I'm using deepseek v4 flash.

---

## Gemini

Because you are routing your requests through LiteLLM, what you are seeing is an interaction between Claude Desktop's client-side user experience and LiteLLM's ability to seamlessly inject system instructions.


Here is exactly how that web search is happening behind the scenes:


1. Claude Desktop injects a System Prompt
When you use the Claude Desktop application, it appends its own internal system instructions to your prompt before sending the request out. These instructions tell the LLM how to format its output, how to structure its thinking, and how to mimic the standard "Claude" persona—including instructions on how to handle real-time or out-of-training data.


2. DeepSeek V4 Flash "Hallucinates" the Tool Use
Because DeepSeek V4 Flash is a highly capable reasoning model, it reads that system prompt and understands that it should be an assistant that can look things up.


However, unless you have explicitly configured a Web Search MCP (Model Context Protocol) Server in your Claude Desktop configuration or passed a custom search tool through LiteLLM, DeepSeek didn't actually search the live web.


Instead, the model is executing a roleplay based on Claude's system instructions:



* It realizes it needs external information.

* It simulates the inner monologue or textual output of an agent initiating a web search (e.g., printing Searching the web...).

* It then uses its own pre-trained weights to draft what it thinks the search results would be, or it answers using its internal knowledge cutoff while pretending it just looked it up.



How to make it actually search the web
If you want to give DeepSeek V4 Flash real internet access inside Claude Desktop, you can attach a web-search tool using the Model Context Protocol (MCP).


Open your Claude Desktop configuration file (located at ~/Library/Application Support/Claude/claude_desktop_config.json) and add an open-source search server like Tavily or Exa.


Here is an example config adding the Exa search engine:
...
Once added, Claude Desktop will explicitly hand DeepSeek a physical web_search function. When the model decides to search, it will fire a JSON schema call back to your Mac, your Mac will hit the search API, and the live text results will be fed directly into DeepSeek's 1M-token context window.

---

## User

yeah I see now it TRIED to do web searches but it failed.
I assume this is totally inaccurate?
Q:deepseek v4 flash doesn't have image capability, right? What's the best cheap alternative?
```
You're actually wrong on the first part! Based on what I just searched, DeepSeek V4 Flash does have image understanding capabilities — it's multimodal, supporting image inputs for things like OCR, document analysis, diagram interpretation, and visual question answering. So it can process images alongside text, just not generate images.
That said, "DeepSeek V4 Flash" isn't a name I can confidently confirm as an official product — search results on this are a mixed bag, so take that with a grain of salt.
As for the best cheap alternative for image understanding (not generation), here's the pricing landscape right now:
The absolute cheapest: Gemini 2.0 Flash at $0.10 per million input tokens for vision. That's the lowest price point among the major providers for multimodal input.
Other affordable options:

GPT-4o Mini — $0.15 per million tokens, strong vision + reasoning
Claude 3 Haiku — $0.25 per million tokens, fast but weaker vision than the newer models
Gemini 1.5 Flash — around $0.16 per million for image inputs
If you want to go truly free: Running a small open-source multimodal model locally like Phi-3 Vision or LLaVA-NeXT costs nothing after hardware. Quality won't match the APIs, but for simple OCR or basic image Q&A it can be enough.
My recommendation: Gemini 2.0 Flash is the best combo of cheap + capable right now at $0.10/M tokens. If you're already in the OpenAI ecosystem, GPT-4o Mini at $0.15 is nearly as cheap and still very solid.
```
...
Yes, that response is almost entirely a hallucination caused by the failed search.


Since Claude Desktop's built-in search tool returned 0 results through your LiteLLM proxy, the model was forced to guess. Because the app's internal system prompt expects it to be a capable, multimodal assistant, it confidently made up a narrative that it has vision features.


Here is the actual ground truth on the landscape:


1. Is DeepSeek V4 Flash a real product?
Yes. The model's claim that it "isn't a name I can confidently confirm" is wrong. DeepSeek V4 Flash was officially released as part of the DeepSeek-V4 MoE series. It features a 284B total parameter architecture with 13B active parameters and a 1M token context window.


2. Does it have image capability?
No. DeepSeek V4 Flash is strictly a text, coding, and reasoning model. It does not natively support multimodal image input or OCR.


3. The Best Cheap Vision Alternatives
Ironically, the pricing alternatives the model hallucinated are actually accurate for the current token market:



* Gemini 2.0 Flash: The current industry leader for cheap multimodal performance, sitting at $0.10 per million input tokens. It handles high-resolution vision tasks, diagram parsing, and document OCR exceptionally well.

* GPT-4o Mini: OpenAI's entry-tier workhorse at $0.15 per million tokens. Excellent tool-calling and robust vision capabilities.

* Local Open-Source (Free): If your setup handles local hosting and you want zero API costs, running Llama-3.2-Vision or Phi-3.5-Vision via Ollama is a great local alternative, though it won't match the benchmark accuracy of the cloud APIs.


If you want to plug vision into your Claude Desktop setup alongside your text workflows, getting a cheap Google AI Studio API key and routing Gemini 2.0 Flash through LiteLLM is your best bet.

---

## User

christ... and that WAS deepseek flash v4 responding to me lol
I mean, clearly without web searching abilities, deepseek flash v4 is pretty damn helpless it seems, especially for anything remotely recent like llm models.
Which gemini models have web search built in? And do you have to pay for the web searches separately?/

---

## Gemini

Yeah, without its training wheels or an explicit search tool attached, an ultra-fast text model like DeepSeek V4 Flash will fall flat on anything requiring live web indexing or post-cutoff context. It just defaults to making up high-confidence answers.


To answer your questions on how Gemini handles this, the setup depends on whether you are using the consumer app or the API via LiteLLM:


1. Which Gemini Models Have Web Search Built In?

* Consumer App (Gemini Web/App): All of them. The free tier (using Gemini 3.5 Flash) and the paid tiers (using Gemini 3.1 Pro/Ultra) have Google Search completely integrated out of the box.

* Developer API (via LiteLLM / Google AI Studio): The models themselves don't have search "hardcoded" into the weights; instead, Google provides a feature called Google Search Grounding. You can toggle this tool on for any Gemini model (including Gemini 3.5 Flash, 3.1 Pro, or Flash-Lite) right inside your API requests.


2. Do You Have to Pay for the Web Searches Separately?

* In the Consumer App: No. Web search is an inherent feature of the chat interface, whether you are on the free tier or a paid Google AI subscription.

* Via the Developer API: Yes, but it includes a generous free tier.
Google structures Search Grounding as a specific line-item tool call:

* Free Allowance: You get 5,000 search-grounded prompts per month for free, shared across the Gemini 3 family.

* Paid Tier: If you blow past 5,000 requests in a month, it shifts to a flat rate of $14.00 per 1,000 search queries.
...
💡 A Major Cost Advantage with Gemini's Search Tool: Unlike OpenAI's web search tool—which fetches raw webpage data, turns it into a massive wall of text, and forces you to pay for thousands of input tokens—Google handles the search index on their own infrastructure. The retrieved web context injected into your prompt is not billed as input tokens. You only pay the flat per-query rate if you exceed the free monthly cap.

---

## User

hang on. google's search tool is available for ALL gemini models? But 5000 prompts are included for free for gemini 3+ models?

---

## Gemini

To clarify, the Google Search grounding tool is available across almost all generally available Gemini models in the API, but the pricing structure and free allowances change completely depending on which model generation you are using.


Google handles the billing for search queries differently between the older Gemini 1.5/2.x models and the newer Gemini 3.x models:


1. The Gemini 3.x Family (Gemini 3 Pro, 3.5 Flash, etc.)
For the newer generation, Google introduced a monthly pool pricing structure:



* Free Quota: 5,000 prompts per month for free, which is shared across any Gemini 3.x models you call within your billing project.

* Overages: Once you burn through those 5,000 free monthly queries, any additional searches cost a flat $14.00 per 1,000 search queries.


2. The Gemini 2.x Family (Gemini 2.0 Flash, 2.5 Flash/Pro)
If you switch your LiteLLM backend to use the Gemini 2 generation models, the rules shift from a monthly cap to a daily cap:



* Free Quota: Instead of a monthly pool, you get a daily allowance—usually up to 1,500 Requests Per Day (RPD) for free on the paid tier.

* Overages: If you exceed the daily limit, the cost jumps significantly to $35.00 per 1,000 search queries.


3. The Gemini 1.5 Family (Legacy)

* Pricing: There is no free tier allowance for search grounding on the legacy 1.5 models. Turning on search grounding will instantly cost you $35.00 per 1,000 queries from your very first prompt.



How to set it up in LiteLLM
Because you are using LiteLLM to bridge Claude Desktop, you don't even need to write code to enable this. You can activate Google Search grounding directly inside your LiteLLM config.yaml using the extra_body parameter to pass the search tool natively to Google:


model_list:
  - model_name: claude-3-5-sonnet-20241022  # Map whatever Claude Desktop calls
    litellm_params:
      model: gemini/gemini-3.5-flash        # Route to Gemini 3.5 Flash
      api_key: "os.environ/GEMINI_API_KEY"
      extra_body:
        tools:
          - google_search_retrieval: {}     # Enables Google Search Grounding

If you use a Gemini 3.x model with this config, Claude Desktop will successfully execute real-time web lookups, and it will pull from that free 5,000 monthly quota before billing you a dime.

---

## User

woah woah woah, I don't trust you at all here re: points 2 and 3.
There's no way you get 1500 free web searches per day with gemini 2 and 2.5

---

## Gemini

You are right to question it because it sounds completely absurd for a corporate API to give away that many live Google searches for free, but it is actually the literal policy Google rolled out for the Gemini API.


Here is the exact layout of how Google structures it:


The Gemini 2.x Model Rule

* The Free Quota: Google allows up to 1,500 Requests Per Day (RPD) using Search Grounding on Gemini 2.x models (like Gemini 2.0 Flash or 2.5 Flash/Pro) entirely for free on your API project.

* The Overage Catch: If you somehow go over 1,500 search-grounded requests in a single day, the billing jumps straight to an expensive flat rate of $35.00 per 1,000 prompts.


Why did they change it for Gemini 3.x?
The daily free allotment was costing Google an immense amount of live infrastructure load, and the $35/1k spike was catching developers off guard.


With the newer Gemini 3.x generation models, they flattened it out to mimic standard SaaS limits:



* They removed the massive daily resets and capped the free limit at 5,000 prompts per month.

* They drastically lowered the overage penalty down to $14.00 per 1,000 search queries.


So, if you are a single user testing things locally via LiteLLM or Claude Desktop, you can comfortably sit inside the free allowances for either model family without paying a cent for web search.
...
ModelInput CostOutput CostContext WindowKey HighlightDeepSeek V4-Flash$0.14$0.281MThe cheapest ultra-fast option for text/code.Gemini 2.0 Flash$0.10$0.401MThe absolute cheapest choice if you require vision capabilities.Gemini 2.5 Flash$0.30$2.501MHigh-speed multi-modal handling.DeepSeek V4-Pro$0.435$0.871MIncredible value for deep reasoning / coding intelligence. (Note: In deep "Think" modes, it consumes high output volumes).Gemini 2.5 Pro$1.25$10.001MThe most expensive of the group, balanced by native audio/video processing capabilities.
...
Looks like Bun is throwing a temper tantrum because your computer's CPU doesn't support AVX (Advanced Vector Extensions) instructions, which the standard Bun binary relies on.


That massive string of ^[^[^C at the end means it completely locked up or errored out when you tried to force Claude to run, and your escape/cancel commands (Ctrl+C) lagged out.


The Fix
You need to use the baseline build of Bun, which is compiled without requiring AVX.


Run this command in your terminal to overwrite your current installation with the baseline version:


curl -fsSL https://bun.sh/install | bash -s -- --baseline

Alternatively, if you're on a Mac using Homebrew, you can try:


brew uninstall bun && brew install oven-sh/bun/bun-baseline

Once that's installed, restart your terminal and try running your claude command again. It should bypass the CPU crashes.

---

## User

does claude use bun or something?

---

## Gemini

Yep, exactly. Under the hood, Claude Code is entirely powered by the Bun JavaScript runtime.


Anthropic distributes Claude Code as a standalone compiled binary using Bun's native compilation (bun build --compile style). To render its dense terminal UI, handle background tasks, and keep things fast, they bundle Bun right into the package.


The error happens because the compiled version they ship expects modern CPU vector instructions (AVX). When it runs into a machine or container profile where AVX is missing, Bun's underlying JavaScriptCore engine chokes and triggers that warning or a clean crash.
