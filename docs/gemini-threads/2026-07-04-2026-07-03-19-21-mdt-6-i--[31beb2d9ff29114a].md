...
StepActionAgent Context Burden1. InitializationAgent reads local project files + a lightweight Global Index File.Minimal (~few hundred tokens)2. DiscoveryAgent identifies an existing component in the index that matches your requirements.None3. RetrievalAgent uses a custom tool to pull only that file's contents into context.Targeted (Only the size of the asset)4. ImplementationAgent integrates the recalled asset into your new project.Optimal
